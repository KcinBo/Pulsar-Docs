...
The image files are included inside the visible root-level `assets/` folder. Markdown pages link to them with repository-relative paths. Keep this folder in the repository and preserve its name. GitBook will upload and render the assets through Git Sync or a GitHub web upload.
...
Formula pages use GitBook's native KaTeX syntax: `$...$` for inline expressions and `$$...$$` for centered display equations. Keep the dollar-sign delimiters and TeX backslashes unchanged when copying Markdown. No external math plugin is required.
